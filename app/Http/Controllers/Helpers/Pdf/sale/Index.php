...
class Index extends fpdf {
	...
	function __construct($sales_id, $for_commerce, $afip_ticket) {
		...
		$this->header = new Header($afip_ticket);
		$this->footer = new Footer($afip_ticket);
 ...
 // 			$this->SetY($this->Y);
	...
	function printArticle($article) {
		if ($this->afip_ticket) {
	        ...
	        $this->Cell(20,6,$article->pivot->price,0,0,'R');
	        $this->Cell(15,6,'0.00',0,0,'R');
	        $this->Cell(20,6,Numbers::price(PdfArticleHelper::getSubTotalPrice($article)),0,0,'R');
	        $this->Cell(20,6,Numbers::price(PdfArticleHelper::getSubTotalPrice($article)),0,0,'C');
